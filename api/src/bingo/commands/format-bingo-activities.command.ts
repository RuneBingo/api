...
export class FormatBingoActivitiesHandler {
  ...
  async execute(command: FormatBingoActivitiesCommand): Promise<FormatBingoActivitiesResult> {
    ...
    const activities = command.activities.map((activity) => {
      ...
      const action = this.i18nService.t(
        ('bingo.activityActions.' + activity.key.split('bingo.')[1]) as keyof I18nTranslations,
      );
      const title = this.i18nService.t('bingo.formatBingoActivities.title', {
        args: { action: action, username: userDto?.username },
      });
